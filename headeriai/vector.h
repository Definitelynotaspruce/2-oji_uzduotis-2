...
namespace rice
{
    ...
    {
        if (vec_sz < other.vec_sz)
        {
            vec_sz = other.vec_sz << 2;
            reallocate();
        }
        for (size_type i = 0; i < other.vec_sz; ++i)
            buffer[i] = std::move(other.buffer[i]);
    ...
    {
       // std::cout << "it = " << (buffer + vec_sz) << std::endl;
    ...
    typename vector<T>::iterator vector<T>::emplace(vector<T>::const_iterator pos, Args &&... args)
    {
        ...
        if (vec_sz == cap_sz)
            grow_reallocate(cap_sz);
        std::uninitialized_move_n(it, (vec_sz - (pos - buffer)) * sizeof(T), it + 1);
    ...
    template <class T>
    typename vector<T>::iterator vector<T>::insert(vector<T>::const_iterator pos, T &&val)
    {
        iterator it = &buffer[pos - buffer];
        if (vec_sz == cap_sz)
            grow_reallocate(cap_sz);
    ...
    typename vector<T>::iterator vector<T>::insert(vector<T>::const_iterator pos, vector<T>::size_type cnt, const T &val)
    {
        ...
        std::uninitialized_move_n(f, (vec_sz - (pos - buffer)) * sizeof(T), f + cnt);
        vec_sz += cnt;
    ...
    typename vector<T>::iterator vector<T>::erase(vector<T>::const_iterator pos)
    {
        ...
        std::uninitialized_move_n(it + 1, (vec_sz - (pos - buffer) - 1) * sizeof(T), it);
        --vec_sz;
        return it;
    }
    ...
    {
        iterator f = &buffer[first - buffer];
        if (first == last)
            return f;
        for (; first != last; ++first)
            allocator.destroy(first);
        std::uninitialized_move_n(last, (vec_sz - (last - buffer)) * sizeof(T), f);
        vec_sz -= last - first;
        return f;
    }
